...
public class FrameBuffer implements AutoCloseable {
    ...
    public static FrameBuffer create(int width, int height, InternalFormat color) throws Exception {
        return create(width, height, color, Optional.empty());
    }

    public static FrameBuffer create(int width, int height, InternalFormat color, Optional<InternalFormat> depth
    ) throws Exception {
        ...
        Image2D imageColor = new Image2D(width, height, color, TextureFormat.RGB, null, DataType.UNSIGNED_BYTE);
...
//            Image2D imageDepth = new Image2D(width, height, depth.get(), TextureFormat.DEPTH, null,
